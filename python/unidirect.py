...
def plotPrintableFieldenvelopes(zn, s0, s1_norm, s2_norm, s3_norm,
                       varphi_plus, varphi_minus,
                       varphi_plus_text, varphi_minus_text, titletext):
    """
    Plot the extracted Stokes parameters for the pump.
    """
    fig, ax = plt.subplots(3,1,figsize=(5.4,5.0))
    major_x_ticks = np.linspace(0.0, 1.0, 6)
    uselogscale = False
#    ax[0].yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
    if uselogscale:
        ax[0].plot(zn, 10*s0/s0[0], 'k', label="$S_0(z)$")
        ax[0].set_yscale('log')
#        ax[0].yaxis.set_major_formatter(FuncFormatter(lambda y, _: f"{y:.2f}"))
#        ax[0].set_ylim(top=np.max(1.05*s0/s0[0]))
        formatter = ScalarFormatter()
        formatter.set_scientific(False)
        formatter.set_useOffset(False)
        ax[0].yaxis.set_major_formatter(formatter)
        ax[0].set_ylabel('$10\log(S_0(z)/S_0(0))\ \hbox{dB}$')
    else:
        ax[0].plot(zn, s0/s0[0], 'k', label="$S_0(z)$")
        ax[0].set_ylim(bottom=0.0, top=np.max(1.05*s0/s0[0]))
        ax[0].yaxis.set_major_formatter(FuncFormatter(lambda y, _: f"{y:.1f}"))
        ax[0].yaxis.set_major_locator(MaxNLocator(nbins=5, min_n_ticks=5))
        ax[0].set_ylabel('$S_0(z)/S_0(0)$')
    ax[0].autoscale(enable=True, axis='x', tight=True)
    ax[0].set_xticks(major_x_ticks)
...
#    ax[0].set_yticks(np.linspace(0.0, 1.0, 5))
    ax[0].grid(which='both')
#    ax[0].set_title(titletext)
    ax[0].text(0.975, 0.975*np.max(s0/s0[0]), titletext, 
               horizontalalignment="right", verticalalignment="top")

    ax[1].plot(zn, varphi_plus, 'k-', label=varphi_plus_text)
    ax[1].plot(zn, varphi_minus, 'k--', label=varphi_minus_text)
    ax[1].yaxis.set_major_locator(MaxNLocator(nbins=5, min_n_ticks=5))
    ax[1].autoscale(enable=True, axis='x', tight=True)
...
#    ax[2].set_yticks(np.linspace(0.0, 1.0, 5))
    ax[1].grid()
    ax[1].set_ylabel('$\\varphi^{\pm}(z)/\hbox{rad}$')

    ax[2].plot(zn, s1_norm, 'k', linestyle="dotted", label="$S_1(z)/S_0(z)$")
    ax[2].plot(zn, s2_norm, 'k', linestyle="dashed", label="$S_2(z)/S_0(z)$")
    ax[2].plot(zn, s3_norm, 'k', linestyle="solid", label="$S_3(z)/S_0(z)$")
    ax[2].autoscale(enable=True, axis='x', tight=True)
    ax[2].legend(loc="upper right")
    ax[2].set_xticks(major_x_ticks)
    ax[2].set_yticks(np.linspace(-1.0, 1.0, 5))
    ax[2].grid()
#    ax[2].set_ylabel('$S_k(z)/S_0(z)$')
    ax[2].set_xlabel('$z/L$')
...
def graph_06():
    ...
    fig = plotPrintableFieldenvelopes(zn,
                s0_pump, s1_pump_norm, s2_pump_norm, s3_pump_norm,
                varphi3p, varphi3m,
